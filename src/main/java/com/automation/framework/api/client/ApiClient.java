...
public class ApiClient {
  ...
  public Response patch(String path, Object body) {
    ...
    return RestAssured.given(specification).body(body).patch(path);
  }
  ...
  private void applyAuthentication(RequestSpecBuilder builder) {
    ...
    switch (authType) {
      case "bearer" -> {
        LOGGER.info("API authentication strategy set to bearer");
        builder.addHeader("Authorization", "Bearer " + ConfigManager.get("api.auth.token"));
      }
      case "basic" -> {
        LOGGER.info("API authentication strategy set to basic");
        builder.setAuth(
            RestAssured.basic(
                ConfigManager.get("api.auth.username"), ConfigManager.get("api.auth.password")));
      }
