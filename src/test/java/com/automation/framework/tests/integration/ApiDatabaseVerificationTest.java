...
class ApiDatabaseVerificationTest extends BaseDatabaseTest {
  ...
  void apiResponseCanBePersistedAndVerifiedInDatabase() {
    ...
    databaseClient.update(
        "insert into api_audit (audit_id, source, payload_value) values (?, ?, ?)",
        2001,
        "jsonplaceholder",
        email);

    List<Map<String, Object>> rows =
        databaseClient.query("select payload_value from api_audit where audit_id = ?", 2001);
    assertThat(rows)
        .singleElement()
        .extracting(row -> row.get("payload_value"))
  ...
  void apiPayloadAndDatabaseAuditStayAligned() {
    ...
    List<Map<String, Object>> rows =
        databaseClient.query(
            "select source, payload_value from api_audit where audit_id = ?", 3001);
    assertThat(rows)
        .singleElement()
        .satisfies(
            row -> {
